...
try {
    ...
    $statusLines = Invoke-ProbeCommand -Port $serial -Command 'status' -ExpectedPatterns @('^cmd: status$', '^diag0=\d+ xstop=\d+ diag2=\d+ pressed=\d+ conf=\d+ load=\d+ mech=\d+ stall=\d+ source=\d+ force=\d+ state=\d+ homed=\d+ hold=\d+ pos=-?\d+ target=-?\d+ press=-?\d+ cycles=\d+ done=\d+ backoff=\d+ seek=\d+ fault=\d+$')
